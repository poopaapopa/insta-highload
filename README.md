...
Рассчитано путем умножения среднего размера хранилища пользователя на примерное количестов пользователей (3 млрд).

| Тип данных | Общий объем |
| :--- | :--- |
| **Stories** | **846.45 Пбайт** |
| **Сообщения (Direct)** | **801.99 Пбайт** |
| **Посты** | **168.48 Пбайт** |
| **Комментарии** | **35.1 Пбайт** |
| **Профиль** | **1.65 Пбайт** |
| **Лайки** | **1.05 Пбайт** |
| **Подписки** | **0.02 Пбайт** |
| **Итого** | **~1,854.75 Пбайт** **~1.8 Эксабайта** |
...
Для пиковых значений брал **коэффициент 1.5**

| Тип запроса | Средний RPS | Пиковый RPS | Логика расчёта |
| :--- | :--- | :--- | :--- |
| **Просмотр ленты / Stories** (Read) | **243 055** | **364 582** | **DAU \* (Время / 1 мин) / 86400**<br>Пользователь проводит 42 минуты. Делает 1 запрос к API (подгрузка контента/пагинация) в минуту.<br>*(500 млн \* 42) / 86400* |
| **Лайки** (Write) | **48 611** | **72 916** | **Total Likes / 86400**<br>4.2 млрд лайков в день.<br>*(4 200 000 000 / 86400)* |
| **Комментарии** (Write) | **21 990** | **32 985** | **Total Comments / 86400**<br>95 млн постов \* 20 комм. = 1.9 млрд/сутки.<br>*(1 900 000 000 / 86400)* |
| **Загрузка медиа** (Upload) | **4 571** | **6 856** | **(Посты + Stories) / 86400**<br>95 млн постов + ~300 млн Stories = 395 млн загрузок.<br>*(395 000 000 / 86400)* |
...
| **Авторизация** (Auth) | **868** | **1 302** | **DAU \* 15% / 86400**<br>Только 15% пользователей проходят логин.<br>*(500 млн \* 0.15) / 86400* |
| **Регистрация** (Sign Up) | **3.8** | **5.7** | **New Users / 86400**<br>Оценка: 10 млн новых регистраций в месяц (глобальный рост + боты).<br>*(10 000 000 / 30 дней) / 86400* |
