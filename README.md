...
Подводя итоги расчетам, общий объем реляционных данных за 5 лет составит **52.1 Петабайт**. Однако с учетом создания структур для быстрого поиска, итоговый размер базы данных возрастает до **66.4 Петабайт**. 

### Расчёт QPS

Рассчёт нагрузки на чтение/запись (QPS), путём распределения запросов RPS на операции чтения и записи в базу данных по каждому выделенному выше запросу:
